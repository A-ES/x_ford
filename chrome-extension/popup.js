  ...
  return new Promise((resolve) => {
    chrome.storage.sync.get([
      ...
      config = {
        ...
        apiUrl: result.api_url || 'http://localhost:5100',
...
async function saveConfiguration() {
  ...
  const apiUrl = apiUrlInput.value.trim() || 'http://localhost:5100';
...
function openDashboard() {
  chrome.tabs.create({ url: `${config.apiUrl.replace('/api', '')}/dashboard` });
}
