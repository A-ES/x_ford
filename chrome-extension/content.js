...
chrome.storage.sync.get(['api_url', 'keywords', 'monitored_accounts', 'collection_enabled'], (result) => {
  CONFIG.API_URL = result.api_url || 'http://localhost:5100';
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  ...
  if (changes.api_url) {
    CONFIG.API_URL = changes.api_url.newValue;
  }
});
...
function checkRelevance(tweetText, username) {
  ...
  const mentionsMonitoredAccount = CONFIG.MONITORED_ACCOUNTS.some(account => 
  ...
  return mentionsMonitoredAccount || containsKeyword;
}
